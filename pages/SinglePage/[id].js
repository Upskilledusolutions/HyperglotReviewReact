...
import artData from "../api/art";
import data from "../api/design";

export function getServerSideProps(context) {
  const { id, type } = context.query; // Extract id and type from query parameters
  return {
    props: { id, type }, // Pass id and type as props
  };
}

const Page = ({ id, type }) => {
  const data = type === "art" ? artData : designData;
