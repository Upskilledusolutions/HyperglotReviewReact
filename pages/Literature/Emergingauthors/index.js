...
export default function index() {
  ...
  data.filter(data=>{if(data.category === "Emerging Authors"){itemdata.push(data)}})
  ...
  return (
    <div>
        ...
        <div className={styles.bigcontainer}>
            <h1 className={styles.heading}>Emerging Authors</h1>
            <div className={`${styles.flex} ${styles.leftflex}`}>
      <div className={styles.container}>
      <div className={styles.topcontainer}>
        <div className={styles.topimgcontainer}>
          <Image className={styles.topbannerimg} src={staticdata[0].url} width={700} height={400} alt="image" />
        </div>
        <div className={styles.topinfo} key={staticdata[0].id}>
          <h3>{staticdata[0].topic}</h3>
          <p>{staticdata[0].description.slice(0,300)}...</p>
        </div>
      </div>
    {itemdata.reverse().map((data) => {
      return(
